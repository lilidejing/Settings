...
public class HomeSettings extends SettingsPreferenceFragment {
        ...
        for (int i = 0; i < homeActivities.size(); i++) {
            ...
            try {
                ...
                HomeAppPreference pref = new HomeAppPreference(context, activityName, prefIndex,
                        icon, name, this, info);
                pref.setOnPreferenceClickListener(mHomeAppPreferenceClickListener);
                mPrefs.add(pref);
    ...
    class HomeAppPreference extends Preference {
        ...
        public HomeAppPreference(Context context, ComponentName activity,
            ...
            setIcon(icon);
            setTitle(title);
            setSummary(R.string.home_none_setting_summary);
        ...
        protected void onBindView(View view) {
            ...
            CheckBox checkbox = (CheckBox) view.findViewById(R.id.home_app_check);
        ...
        void setChecked(boolean state) {
            if (state != isChecked) {
                isChecked = state;
            	setSummary(isChecked ? R.string.home_default_setting_summary 
                		: R.string.home_none_setting_summary);
                notifyChanged();
            }
        }
    }
    
    private OnPreferenceClickListener mHomeAppPreferenceClickListener = new OnPreferenceClickListener() {
		
		@Override
		public boolean onPreferenceClick(Preference preference) {
			// TODO Auto-generated method stub
			if (preference instanceof HomeAppPreference) {
				if (!((HomeAppPreference) preference).isChecked)
					makeCurrentHome((HomeAppPreference) preference);
				return true;
			}
			return false;
		}
